...
export const SongCard: React.FC<SongCardProps> = ({
  ...
  // Compact View for Rejected Songs
  if (isRejected) {
    return (
      <div className="relative overflow-hidden rounded-lg border border-red-900/30 bg-red-950/10 p-2 flex items-center gap-3 opacity-70 transition-all">
        ...
        <div className="flex-1 min-w-0 flex items-center justify-between gap-2">
          <div className="min-w-0">
            <h3 className="font-semibold text-red-200/50 text-sm truncate line-through decoration-red-500/30">{song.title}</h3>
            <p className="text-slate-600 text-xs truncate">{song.artist}</p>
          </div>
          <div className="px-2 py-0.5 bg-red-900/20 rounded border border-red-900/30">
            <span className="text-[10px] text-red-500/50 font-bold uppercase tracking-wider">Denied</span>
          </div>
        </div>
      </div>
    );
